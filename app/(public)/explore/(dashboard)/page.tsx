  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        // `eventService.getAll` returns a paginated object { count, next, previous, results }
        // but some backends may return a raw array. Normalize to an array here.
        ...
        await Promise.all(
          list.map(async (event) => {
            try {
              ...
              likeStatuses[String(event.id)] = !!status.has_liked;
            } catch (err) {
              likeStatuses[String(event.id)] = false;
            }
          }),
        );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Discover astronomical wonders happening around the world
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredEvents.map((event) => (
                <EventCard
                  key={event.id}
                  event={event}
                  isLiked={isEventLiked(String(event.id))}
